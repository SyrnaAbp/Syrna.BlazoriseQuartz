$(function() {
    let pmService = syrna.blazoriseQuartz.privateMessages.privateMessage;
    let notificationService = syrna.blazoriseQuartz.privateMessageNotifications.privateMessageNotification;
    let detailModal = new abp.ModalManager(abp. appPath + 'BlazoriseQuartz/PrivateMessages/PrivateMessage/DetailModal');
    var widgetManager = new abp.WidgetManager({filterForm: 'PmNotification'});

    $(document.body).on('click', '#pmNotificationShowMore', function () {
        document.location.href = "/BlazoriseQuartz/PrivateMessages/PrivateMessage/Inbox"
    });
