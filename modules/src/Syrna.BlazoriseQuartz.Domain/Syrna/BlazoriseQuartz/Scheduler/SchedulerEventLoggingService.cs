...
public class SchedulerEventLoggingService : BackgroundService, ISchedulerEventLoggingService
{
    ...
    private void _schLisSvc_OnJobScheduled(object sender, Events.EventArgs<ITrigger> e)
    {
        ...
        var tKey = e.Args.Key;
        var log = new ExecutionLog.ExecutionLog
        {
            JobName = jKey.Name,
            JobGroup = jKey.Group,
    ...
    private ExecutionLog.ExecutionLog CreateScheduleJobLogEntry(IJobExecutionContext context,
    ...
    void QueueUpdateTask(ExecutionLog.ExecutionLog log)
    {
        QueueTask(async (repo, cancelToken) =>
        {
            try
            {
                if (!repo.Exists(log))
    ...
    void QueueGetJobKeyAndInsertTask(ExecutionLog.ExecutionLog log)
    {
        QueueTask(async (repo, cancelToken) =>
        {
            try
    ...
    void QueueInsertTask(ExecutionLog.ExecutionLog log)
    {
        QueueTask(async (repo, cancelToken) =>
        {
            try
