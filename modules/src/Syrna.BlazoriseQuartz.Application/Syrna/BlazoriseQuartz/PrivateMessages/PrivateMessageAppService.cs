...
namespace Syrna.BlazoriseQuartz.PrivateMessages
{
        ...
        IPrivateMessageAppService
    {
        ...
        private readonly IStringLocalizer<BlazoriseQuartzResource> _localizer;
        ...
        public PrivateMessageAppService(
            IStringLocalizer<BlazoriseQuartzResource> localizer,
        ...
        public virtual async Task<PrivateMessageDto> GetAsync(Guid id)
        {
            using (_dataFilter.Disable<ISoftDelete>())
            {
                ...
                await AuthorizationService.CheckAsync(message,
                    ...
                    { Name = BlazoriseQuartzPermissions.PrivateMessages.Default });

                return await MapToDtoAndLoadMoreInfosAsync(message);
            }
        }

        [Authorize(BlazoriseQuartzPermissions.PrivateMessages.Default)]
        ...
        public virtual async Task DeleteManyAsync(IEnumerable<Guid> ids)
        {
            ...
            foreach (var message in messageList)
            {
                await AuthorizationService.CheckAsync(message,
                    new OperationAuthorizationRequirement
                    { Name = BlazoriseQuartzPermissions.PrivateMessages.Delete });
        ...
        public virtual async Task SetReadAsync(IEnumerable<Guid> ids)
        {
            ...
            foreach (var message in messageList)
            {
                await AuthorizationService.CheckAsync(message,
                    new OperationAuthorizationRequirement
                    { Name = BlazoriseQuartzPermissions.PrivateMessages.SetRead });

                await _privateMessageReceiverSideManager.SetReadAsync(message);
            }
        }

        [Authorize(BlazoriseQuartzPermissions.PrivateMessages.Create)]
